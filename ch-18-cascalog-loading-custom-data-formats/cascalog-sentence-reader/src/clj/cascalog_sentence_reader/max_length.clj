(ns cascalog-sentence-reader.max-length
  (:require
   [cascalog.cascading.util :as util] [cascalog.logic [vars :as v] [ops :as c]] 
   [clojure.string :as s :refer [join]] [cascalog.api :refer :all] 
...
(defn sentence-file
  "Custom scheme for dealing with entire files." [field-names]
...
(defn hfs-sentencefile
  ...
  (let [scheme (-> (:outfields (apply array-map opts) Fields/ALL)
                   (sentence-file))] (apply hfs-tap scheme path opts)))
(defmapcatfn split [line]

  "reads in a line of string and splits it by regex" (s/split line #"[\[\]\\\(\),.)\s]+"))
(defn -main [in out & args]
  (let [; define subquery for average
        counts (<- [?word-count ?sentence]
                   ((hfs-sentencefile in :skip-header? false) ?sentence _) (split ?sentence :> ?word-dirty)
                   (c/count ?word-count))]
    (?<- (hfs-delimited out) [?mymax]
         (counts ?vals ?sentence) (c/max ?vals :> ?mymax))))
(defn -main-get-sentence [in out & args] (let [counts (<- [?word-count ?sentence]
                                                          ((hfs-sentencefile in :skip-header? false) ?sentence _) (split ?sentence :> ?word-dirty)
                                                          (c/count ?word-count))]
                                           (?<- (hfs-delimited out) [?word-count ?sentence] ((c/first-n counts 1
                                                                                                        :sort ["?word-count"]
