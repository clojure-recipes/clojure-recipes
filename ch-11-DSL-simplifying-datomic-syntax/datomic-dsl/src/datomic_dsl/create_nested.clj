(ns datomic-dsl.create-nested 
  (:require 
   [clojure.zip :as zip] [datomic.api :as d]))

(defn map-zip
  "Define a custom zipper for maps similar to zip/vector-zip and zip/seq-zip." 
  [m]
  (zip/zipper
   (fn [x] (or (map? x) (map? (nth x 1)))) (fn [x] (seq (if (map? x) x (nth x 1)))) (fn [x children]
                                                                                      (if (map? x)
                                                                                        (into {} children)
...
corresponding dsl entity syntax."
  ...
  (if (= (type (zip/node zipped-node)) clojure.lang.MapEntry)
    (let [elem (zip/node zipped-node) attr-key (key elem)
          attr-val (val elem)
...
nested schema entities."
  ...
  [schema-name dsl] (filter (comp not nil?)
                            (map #(create-schema-nested-fn schema-name %) (zip-nodes (dsl-zipper dsl)))))
