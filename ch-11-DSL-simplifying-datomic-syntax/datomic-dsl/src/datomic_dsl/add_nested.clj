...
(defn steps-up 
  "Keys to to the top for a given node."
  [node]
  (filter (comp not nil?) 
...
(defn key-sequences
  "get the key sequences from the dsl and filter out the top-level ones and return a set of vectors"
  [dsl]
  (set
   (map vec 
        (map rest 
...
(defn nested-insert-dsl 
  "Step through the existing dsl and add matching db ids to future parent and children entities"
  [schema-name dsl]
...
(defn convert-key-sequence-fn
  ...
  (fn [key-sequence]
    (let [local-datom (get-in nested-dsl key-sequence)
          ...
          local-datom-minus-children (into {} (map replace-children local-datom-minus-dbid))
          parent-name (take-last 1 key-sequence)
          last-two (take-last 2 key-sequence)
          last (first (take-last 1 key-sequence))
          parent-keyword (case (count last-two) 
                           2  (keyword (first last-two) (second last-two)) 
                           1 (keyword (first last-two)))
          new-keys (into {} (map (fn [[k v]] [k (keyword (str last "/" k))]) local-datom-minus-children))
