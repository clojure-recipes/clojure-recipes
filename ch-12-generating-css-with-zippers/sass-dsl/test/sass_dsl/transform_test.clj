(ns sass-dsl.transform-test
  (:require
   [clojure.test :refer :all] [sass-dsl.parse :as parse] [sass-dsl.transform :as transform] [sass-dsl.common-test :as common-test] [sass-dsl.parse-test :as parse-test] [sass-dsl.common-test :as common-test]))
...
(deftest flatten-colon-suffixes-test (testing "Ensure colon suffixes are flat."
                                       (is (=
...
(def sass-constants-nested-tree
  "Given a sass string return a nested vector." (parse/transform-to-sass-nested-tree common-test/basic-css-constants))
...
(deftest remove-constants
  (testing "Ensure we stripped the constant declarations."
    (is (=
         (transform/replace-const-references "border-color: $blue" {"$blue" "#3bbfce" "$margin" "16px"}) "border-color: #3bbfce"))))

(deftest replaced-constants-structure
  (testing "Ensure we replaced the constants structure."
    (is (= (transform/replaced-constants-structure-vec-zip
            sass-constants-stripped-nested-tree
            constants-map)
